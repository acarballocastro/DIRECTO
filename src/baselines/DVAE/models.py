...
class SVAE(nn.Module):
    def __init__(
        self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False
    ):
        ...
        # 0. encoding-related
        self.grue = nn.GRU(
            self.xs, hs, batch_first=True, bidirectional=self.bidir
        )  # encoder GRU
    ...
    def _collate_fn(self, G):
        ...
        for g in G:
            if g.shape[1] < self.max_n - 1:
                padding = torch.zeros(1, self.max_n - 1 - g.shape[1], g.shape[2]).to(
                    self.get_device()
                )
                padding[0, :, self.START_TYPE] = (
                    1  # use start type's bit to indicate padding
                )
                # nodes (since start types are never predicted)
                g = torch.cat([g, padding], 1)
            if g.shape[2] < self.xs:
                padding = torch.zeros(1, g.shape[1], self.xs - g.shape[2]).to(
                    self.get_device()
                )
                g = torch.cat(
                    [g, padding], 2
                )  # pad zeros to indicate no connections to padding
                # nodes
            G_new.append(g)
                ...
                if vj == self.max_n - 1:
                    ...
                    if stochastic:
                        new_type = np.random.choice(
                            range(self.nvt), p=type_probs[gi][vj - 1]
                        )
                    else:
...
class SVAE_oneshot(SVAE):
    def __init__(
        self, max_n, nvt, START_TYPE, END_TYPE, hs=1002, nz=112, bidirectional=False
    ):
        super(SVAE_oneshot, self).__init__(
            max_n, nvt, START_TYPE, END_TYPE, hs, nz, bidirectional
        )
    ...
    def _decode(self, z):
        ...
        edge_scores = self.sigmoid(
            scores[:, :, self.nvt :]
        )  # batch * max_n-1 * max_n-1
        return type_scores, edge_scores
...
class SVAE_GraphRNN(SVAE):
    def __init__(
        ...
        super(SVAE_GraphRNN, self).__init__(
            max_n, nvt, START_TYPE, END_TYPE, hs, nz, bidirectional
        )
        ...
        )  # encoder GRU (graph level)
        self.grud_edge = nn.GRU(
            1,
            hs,
            num_layers=self.num_layers,
            batch_first=True,
            bidirectional=self.bidir,
        )  # encoder GRU (edge level)
    ...
    def _decode(self, z):
        ...
        H0_graph = (
            H0.unsqueeze(0).expand(self.num_dirs * self.num_layers, -1, -1).contiguous()
        )
        input_graph_level = self._get_zeros(len(z), self.xs).unsqueeze(
            1
        )  # batch * 1 * xs
        type_scores, edge_scores = [], []
        for vi in range(self.max_n - 1):
            output_graph_level, _ = self.grud(
                input_graph_level, H0_graph
            )  # batch * 1 * (hs*num_dirs)
            if self.bidir:
                output_graph_level = self.hg_unify(output_graph_level)  # batch * 1 * hs
            H0_graph = output_graph_level.permute(1, 0, 2)  # 1 * batch * hs
            H0_graph = H0_graph.expand(
                self.num_dirs * self.num_layers, -1, -1
            ).contiguous()
            ...
            for ej in range(self.max_n - 1):
                ...
                edge_score_j = self.sigmoid(
                    self.add_edge(output_edge_level)
                ).detach()  # batch * 1 * 1
                # sample edge j of node i
                edge_score_j = np.random.random_sample((len(z), 1, 1)) < edge_score_j
                edge_score_j = edge_score_j.type(torch.FloatTensor).to(
                    self.get_device()
                )
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        Input_graph_level = torch.cat(
            [self._get_zeros(len(z), self.xs).unsqueeze(1), Input_graph_level], 1
        )  # pad initial zeros
        H0_graph = (
            H0.unsqueeze(0).expand(self.num_dirs * self.num_layers, -1, -1).contiguous()
        )
        Output_graph_level, _ = self.grud(
            Input_graph_level, H0_graph
        )  # batch * max_n-1 * (hs*num_dirs)
        if self.bidir:
            Output_graph_level = self.hg_unify(
                Output_graph_level
            )  # batch * max_n-1 * hs
        ...
        Input_edge_level = torch.cat(
            [self._get_zeros(len(z), self.max_n - 1).unsqueeze(2), Input_edge_level], 2
        )  # pad initial zeros
        Input_edge_level = Input_edge_level.reshape(
            -1, self.max_n - 1, 1
        ).contiguous()  # (batch * max_n-1) * max_n-1 * 1
        Output_edge_level, _ = self.grud_edge(
            Input_edge_level, H0_edge
        )  # (batch * max_n-1) * max_n-1 * (hs * num_dirs)
        if self.bidir:
            Output_edge_level = self.hv_unify(
                Output_edge_level
            )  # (batch * max_n-1) * max_n-1 * hs
        edge_scores = self.sigmoid(
            self.add_edge(Output_edge_level)
        )  # (batch * max_n-1) * max_n-1 * 1
...
class SVAE_GraphRNN_BFS(SVAE):
    def __init__(
        ...
        super(SVAE_GraphRNN_BFS, self).__init__(
            max_n, nvt, START_TYPE, END_TYPE, hs, nz, bidirectional
        )
        self.num_dirs = 2 if self.bidir else 1
        self.num_layers = 1
    ...
    def _collate_fn(self, G):
        ...
        for g in G:
            ...
            g = self.adjfeat_to_G(
                *self.bfs(adj, feat)
            )  # 1 * n_vertex * (n_types + n_vertex)
            if g.shape[1] < self.max_n:
                padding = torch.zeros(1, self.max_n - g.shape[1], g.shape[2]).to(
                    self.get_device()
                )
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        Input_edge_level = G_true[:, :, self.nvt : -1]
        Input_edge_level = torch.cat(
            [self._get_zeros(len(z), self.max_n).unsqueeze(2), Input_edge_level], 2
        )  # pad initial zeros
        Input_edge_level = Input_edge_level.reshape(
            -1, self.max_n, 1
        ).contiguous()  # (batch * max_n) * max_n * 1
        Output_edge_level, _ = self.grud_edge(
            Input_edge_level, H0_edge
        )  # (batch * max_n) * max_n * (hs * num_dirs)
        if self.bidir:
            Output_edge_level = self.hv_unify(
                Output_edge_level
            )  # (batch * max_n) * max_n * hs
        edge_scores = self.sigmoid(
            self.add_edge(Output_edge_level)
        )  # (batch * max_n) * max_n * 1
...
class DVAE(nn.Module):
    def __init__(
        ...
        self.fc3 = nn.Linear(nz, hs)  # from latent z to initial hidden state h0
        self.add_vertex = nn.Sequential(
            nn.Linear(hs, hs * 2), nn.ReLU(), nn.Linear(hs * 2, nvt)
        )  # which type of new vertex to add f(h0, hg)
        self.add_edge = nn.Sequential(
            nn.Linear(hs * 2, hs * 4), nn.ReLU(), nn.Linear(hs * 4, 1)
        )  # whether to add edge between v_i and v_new, f(hvi, hnew)

        # 2. gate-related
        self.gate_forward = nn.Sequential(nn.Linear(self.vs, hs), nn.Sigmoid())
        self.gate_backward = nn.Sequential(nn.Linear(self.vs, hs), nn.Sigmoid())
    ...
    def _propagate_to(self, G, v, propagator, H=None, reverse=False):
        ...
        if H is None:
            ...
            if max_n_pred == 0:
                ...
                H_pred = [
                    torch.cat(
                        h_pred + [self._get_zeros(max_n_pred - len(h_pred), self.vs)], 0
                    ).unsqueeze(0)
    ...
    def _propagate_from(self, G, v, propagator, H0=None, reverse=False):
        ...
        Hv = self._propagate_to(
            G, v, propagator, H0, reverse=reverse
        )  # the initial vertex
        ...
        for idx in range(1, self.max_n):
            ...
            if idx == self.max_n - 1:  # force the last node to be end_type
                ...
                if stochastic:
                    ...
                    new_types = [
                        np.random.choice(range(self.nvt), p=type_probs[i])
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):  # range(1, self.max_n):
            ...
            for i, g_true in enumerate(G_true):
                true_edges.append(
                    g_true.get_adjlist(igraph.IN)[v_true]
                    if v_true < g_true.vcount()
                    else []
                )
            edge_scores = []
...
class DVAE_GCN(DVAE):
    ...
    def _get_graph_state(self, G, decode=False, start=0, end_offset=0):
        ...
        for g in G:
            hg = torch.cat(
                [g.vs[i]["H_forward"] for i in range(start, g.vcount() - end_offset)], 0
            ).unsqueeze(
                0
            )  # 1 * n * hs
            if g.vcount() < max_n_nodes:
                hg = torch.cat(
                    [
                        hg,
                        torch.zeros(1, max_n_nodes - g.vcount(), hg.shape[2]).to(
                            self.get_device()
                        ),
                    ],
                    1,
    ...
    def _GCN_propagate_to(self, G, v, lv=0):
        ...
        if self.bidir:  # ignore edge directions, accept all neighbors' messages
            H_nei = [
                [self._get_feature(g, v, lv) / (g.degree(v) + 1)]
                + [
                    self._get_feature(g, x, lv)
                    / math.sqrt((g.degree(x) + 1) * (g.degree(v) + 1))
            ...
            H_nei = [
                [self._get_feature(g, v, lv) / (g.indegree(v) + 1)]
                + [
                    self._get_feature(g, x, lv)
                    / math.sqrt((g.outdegree(x) + 1) * (g.indegree(v) + 1))
        ...
        H_nei = [
            torch.cat(
                h_nei + [self._get_zeros(max_n_nei - len(h_nei), h_nei[0].shape[1])], 0
            ).unsqueeze(0)
            for h_nei in H_nei
        ]  # pad all to same length
    ...
    def encode(self, G):
        ...
        Hg = self._get_graph_state(
            G, start=1, end_offset=1
        )  # does not use the dummy input
        # and output nodes
        mu, logvar = self.fc1(Hg), self.fc2(Hg)
        return mu, logvar
...
class DVAE_BN(DVAE):
    def __init__(
        ...
        self.gate_forward = nn.Sequential(nn.Linear(self.nvt, hs), nn.Sigmoid())
        self.gate_backward = nn.Sequential(nn.Linear(self.nvt, hs), nn.Sigmoid())
        self.add_edge = nn.Sequential(
            nn.Linear(hs * 3, hs), nn.ReLU(), nn.Linear(hs, 1)
        ...
        for g in G:
            ...
            if self.bidir and not decode:  # decoding never uses backward propagation
                hg_b = torch.cat(
                    [
                        g.vs[i]["H_backward"]
                        for i in range(start, g.vcount() - end_offset)
                    ],
                    0,
                ).unsqueeze(0)
    ...
    def _propagate_to(self, G, v, propagator, H=None, reverse=False):
        ...
        if reverse:
            H_name = "H_backward"  # name of the hidden states attribute
            H_pred = [
                [self._one_hot(g.vs[x]["type"], self.nvt) for x in g.successors(v)]
                for g in G
            ]
            gate, mapper = self.gate_backward, self.mapper_backward
        else:
            H_name = "H_forward"  # name of the hidden states attribute
            H_pred = [
                [self._one_hot(g.vs[x]["type"], self.nvt) for x in g.predecessors(v)]
                for g in G
            ]
            gate, mapper = self.gate_forward, self.mapper_forward
        # if h is not provided, use gated sum of v's predecessors' states as the input hidden state
        if H is None:
            ...
            if max_n_pred == 0:
                ...
                H_pred = [
                    torch.cat(
                        h_pred + [self._get_zero_x((max_n_pred - len(h_pred)))], 0
                    ).unsqueeze(0)
                    for h_pred in H_pred
                ]
...
class DVAE_fast(DVAE):
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        res += F.cross_entropy(type_scores.transpose(1, 2), true_types, reduction="sum")
        true_edges = torch.FloatTensor(
            [
                np.pad(
                    np.array(g_true.get_adjacency().data).transpose()[1:, :-1],
                    (
                        (0, self.max_n - g_true.vcount()),
                        (0, self.max_n - g_true.vcount()),
                    ),
                    mode="constant",
                    constant_values=(0, 0),
        ...
        for gi in range(len(type_scores)):
            ...
            for vi in range(1, self.max_n):
                if vi == self.max_n - 1:
                    ...
                    if stochastic:
                        new_type = np.random.choice(
                            range(self.nvt), p=type_probs[gi][vi - 1]
                        )
                    else:
...
class DVAE_DeepGMG(nn.Module):
    def __init__(
        ...
        # 2. gate-related
        self.gate = nn.Sequential(nn.Linear(self.vs, self.gs), nn.Sigmoid())
        self.mapper = nn.Sequential(
            nn.Linear(self.vs, self.gs, bias=False),
        )  # disable bias to ensure padded zeros also mapped to zeros
        self.gate_init = nn.Sequential(nn.Linear(self.vs, self.gs), nn.Sigmoid())
    ...
    def _propagate_to(self, G, v, propagator, t=0):
        ...
        if self.bidir:  # ignore edge directions, accept all neighbors' messages
            ...
            H_nei = [
                [g.vs[x]["H_forward"] for x in g.predecessors(v)]
                + [g.vs[x]["H_forward"] for x in g.successors(v)]
                for g in G
            ]
            E_nei = [
        ...
        H_nei = [
            torch.cat(
                h_nei + [self._get_zeros(max_n_nei - len(h_nei), self.hs)], 0
            ).unsqueeze(0)
            for h_nei in H_nei
        ]  # pad all to same length
        H_nei = torch.cat(H_nei, 0)  # batch * max_n_nei * hs
        E_nei = [
            torch.FloatTensor(e_nei + [0] * (max_n_nei - len(e_nei)))
            .unsqueeze(0)
            .unsqueeze(2)
            .to(self.get_device())
            for e_nei in E_nei
        ...
        Av = self.fe(
            torch.cat([H_nei, E_nei, Hv_expand], -1)
        )  # batch * max_n_nei * 2hs+1
    ...
    def decode(self, z, stochastic=True):
        ...
        for idx in range(1, self.max_n):
            ...
            edge_scores = []
            G_tmp = [
                g for i, g in enumerate(G) if not finished[i]
            ]  # the remaining graphs to add new edges
            count = 0
            while G_tmp:
                count += 1
                if not self.fast:
                ...
                add_score = self.sigmoid(
                    self.add_edge(torch.cat([Hg, H], 1))
                )  # |G_tmp| * 1
                if stochastic:
                ...
                G_tmp = [
                    g for i, g in enumerate(G_tmp) if decisions[i, 0]
                ]  # the remaining graphs to add new edges
                if not G_tmp or count > idx:
                    break

                Hv = torch.cat(
                    [self._get_vertex_state(G_tmp, v).unsqueeze(1) for v in range(idx)],
                    1,
                )  # |G_tmp| * idx * hs
                H = self._get_vertex_state(G_tmp, idx).unsqueeze(1).expand_as(Hv)
                edge_score = self.select_node(torch.cat([Hv, H], -1)).squeeze(
                    -1
                )  # |G_tmp| * idx
                if stochastic:
                    edge_prob = F.softmax(edge_score, 1).cpu().detach().numpy()
                    new_edge = [
                        np.random.choice(range(idx), p=edge_prob[i])
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):
            ...
            while graph_idx:
                ...
                Hg = self._get_graph_state(G)
                H = self._get_vertex_state(G, v_true)
                add_score = self.sigmoid(self.add_edge(torch.cat([Hg, H], 1)))[
                    graph_idx
                ]  # |graph_idx| * 1
                add_truth = [1.0 if true_edges[i] else 0.0 for i in graph_idx]
                add_truth = torch.tensor(add_truth).to(self.get_device())
                addll = -F.binary_cross_entropy(
                    add_score.squeeze(1), add_truth, reduction="sum"
                )

                Hv = torch.cat(
                    [self._get_vertex_state(G, v).unsqueeze(1) for v in range(v_true)],
                    1,
                )  # batch * v_true * hs
