...
class LayerDAGNodeCountDataset(LayerDAGBaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.conditional:
            ...
            return (
                self.input_src[input_e_start:input_e_end],
    ...
    def __init__(self, dag_dataset, conditional=False, get_marginal=True):
        ...
        if get_marginal:
            ...
            for f in range(num_feats):
                ...
                assert (
                    unique_x_n_f.max().item() == len(x_n_count_f) - 1
...
def collate_common(src, dst, x_n, abs_level, rel_level):
    num_nodes_cumsum = torch.cumsum(
        torch.tensor([0] + [len(x_n_i) for x_n_i in x_n]), dim=0
    )
    ...
    for i in range(batch_size):
        ...
        gids.append(
            torch.ones(num_nodes_cumsum[i + 1] - num_nodes_cumsum[i]).fill_(i).long()
        )
...
def collate_node_pred(data):
    ...
    num_query_cumsum = torch.cumsum(
        torch.tensor([0] + [len(z_t_i) for z_t_i in batch_z_t]), dim=0
    )
    query2g = []
    for i in range(batch_size):
        query2g.append(
            torch.ones(num_query_cumsum[i + 1] - num_query_cumsum[i]).fill_(i).long()
        )
...
def collate_edge_pred(data):
    ...
    num_nodes_cumsum = torch.cumsum(
        torch.tensor([0] + [len(x_n_i) for x_n_i in batch_x_n]), dim=0
    )
