...
__all__ = ["DiscreteDiffusion", "EdgeDiscreteDiffusion"]


class DiscreteDiffusion(nn.Module):
    def __init__(self, marginal_list, T, s=0.008):
        """
        Parameters
        ----------
        ...
        for marginal_d in marginal_list:
            ...
            self.I_list.append(
                nn.Parameter(torch.eye(num_classes_d), requires_grad=False)
            )
            marginal_d = marginal_d.unsqueeze(0).expand(num_classes_d, -1).clone()
    ...
    def apply_noise(self, z, t=None):
        ...
        for d in range(D):
            ...
            z_one_hot_d = F.one_hot(
                z[:, d], num_classes=self.num_classes_list[d]
            ).float()
...
class EdgeDiscreteDiffusion(nn.Module):
    def __init__(self, avg_in_deg, T, s=0.008):
        super().__init__()
    ...
    def get_Qs(self, alpha_t, alpha_bar_s, alpha_bar_t, marginal):
        M = torch.zeros(2)
        M = torch.tensor([1 - marginal, marginal])
