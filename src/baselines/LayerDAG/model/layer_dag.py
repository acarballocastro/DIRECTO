...
class SinusoidalPE(nn.Module):
    def __init__(self, pe_size):
        ...
        if pe_size > 0:
            self.div_term = torch.exp(
                torch.arange(0, pe_size, 2) * (-math.log(10000.0) / pe_size)
            )
    ...
    def forward(self, position):
        ...
        return torch.cat(
            [torch.sin(position * self.div_term), torch.cos(position * self.div_term)],
            dim=-1,
        )
...
class OneHotPE(nn.Module):
    ...
    def forward(self, position):
        ...
        return F.one_hot(
            position.clamp(max=self.pe_size - 1).long().squeeze(-1),
...
class GraphClassifier(nn.Module):
    def __init__(self, graph_encoder, emb_size, num_classes):
        ...
        self.predictor = nn.Sequential(
            nn.Linear(emb_size, emb_size), nn.GELU(), nn.Linear(emb_size, num_classes)
        )
...
class TransformerLayer(nn.Module):
    ...
    def attn(self, q, k, v, num_query_cumsum):
        ...
        for i in range(batch_size):
            q_padded[i, : num_query_nodes[i]] = q[
                num_query_cumsum[i] : num_query_cumsum[i + 1]
            ]
            k_padded[i, : num_query_nodes[i]] = k[
                num_query_cumsum[i] : num_query_cumsum[i + 1]
            ]
            v_padded[i, : num_query_nodes[i]] = v[
                num_query_cumsum[i] : num_query_cumsum[i + 1]
            ]
...
class NodePredModel(nn.Module):
    def __init__(
        ...
        in_hidden_size = (
            in_hidden_size + t_emb_size + len(num_real_classes) * x_n_emb_size
        )
        self.project_h_n = nn.Sequential(
            nn.Linear(in_hidden_size, out_hidden_size), nn.GELU()
        )

        self.trans_layers = nn.ModuleList()
        for _ in range(num_transformer_layers):
            self.trans_layers.append(
                TransformerLayer(out_hidden_size, num_heads, dropout)
            )
...
class EdgePredModel(nn.Module):
    def __init__(self, graph_encoder, t_emb_size, in_hidden_size, out_hidden_size):
        super().__init__()
    ...
    def forward(self, A, x_n, abs_level, rel_level, t, query_src, query_dst, y=None):
        """
        t : torch.tensor of shape (num_queries, 1)
        """
        h_n = self.graph_encoder(A, x_n, abs_level, rel_level, y=y)

        h_e = torch.cat([self.t_emb(t), h_n[query_src], h_n[query_dst]], dim=-1)
...
class LayerDAG(nn.Module):
    def __init__(
        ...
        hidden_size = (
            len(num_x_n_cat) * node_count_encoder_config["x_n_emb_size"]
            + node_count_encoder_config["pe_emb_size"]
            + node_count_encoder_config["y_emb_size"]
        )
        node_count_encoder = BiMPNNEncoder(
            num_x_n_cat, hidden_size=hidden_size, **node_count_encoder_config
        ).to(device)
        self.node_count_model = GraphClassifier(
            node_count_encoder, emb_size=hidden_size, num_classes=max_layer_size + 1
        ).to(device)

        self.node_diffusion = node_diffusion
        hidden_size = (
            len(num_x_n_cat) * node_pred_graph_encoder_config["x_n_emb_size"]
            + node_pred_graph_encoder_config["pe_emb_size"]
            + node_pred_graph_encoder_config["y_emb_size"]
        )
        node_pred_graph_encoder = BiMPNNEncoder(
            num_x_n_cat, hidden_size=hidden_size, **node_pred_graph_encoder_config
        ).to(device)
        ...
        hidden_size = (
            len(num_x_n_cat) * edge_pred_graph_encoder_config["x_n_emb_size"]
            + edge_pred_graph_encoder_config["pe_emb_size"]
            + edge_pred_graph_encoder_config["y_emb_size"]
        )
        edge_pred_graph_encoder = BiMPNNEncoder(
            num_x_n_cat, hidden_size=hidden_size, **edge_pred_graph_encoder_config
        ).to(device)
        self.edge_pred_model = EdgePredModel(
            edge_pred_graph_encoder, in_hidden_size=hidden_size, **edge_predictor_config
        ).to(device)
    ...
    def posterior_edge(
        ...
        for i in range(batch_size):
            ...
            denominator_i[denominator_i == 0.0] = 1.0
            # (num_rows, num_classes, num_classes)
    ...
    def sample_node_layer(
        ...
        node_count_logits = self.node_count_model(
            A, x_n, abs_level, rel_level, A_n2g=A_n2g, y=y
        )
        ...
        if num_new_nodes_total == 0:
            return [torch.LongTensor([]).to(device) for _ in range(batch_size)]
        ...
        h_g = self.node_pred_model.graph_encoder(
            A, x_n, abs_level, rel_level, y=y, A_n2g=A_n2g
        )

        num_query_cumsum = (
            torch.cumsum(torch.tensor([0] + num_new_nodes.squeeze(-1).tolist()), dim=0)
            .long()
            .to(device)
        )
        query2g = []
        for i in range(batch_size):
            query2g.append(
                torch.ones(num_query_cumsum[i + 1] - num_query_cumsum[i])
                .fill_(i)
                .long()
            )
        ...
        for s_x_n in time_x_n_list:
            ...
            x_n_0_logits = self.node_pred_model.forward_with_h_g(
                h_g, x_n_t, t_x_n_tensor, query2g, num_query_cumsum
            )
            ...
            for d in range(D):
                ...
                x_n_t_one_hot_d = F.one_hot(
                    x_n_t[:, d], num_classes=num_classes_list[d]
                ).float()

                x_n_s_probs_d = self.posterior(
    ...
    def sample_edge_layer(
        ...
        for i in range(batch_size):
            ...
            if isolated_mask.any():
                e_t_mask_i[
                    isolated_mask, torch.zeros(int(isolated_mask.sum().item())).long()
                ] = 1
        ...
        batch_edge_index = self.get_batch_A(
            num_nodes_cumsum, edge_index_list, device, return_edge_index=True
        )
        ...
        for s_x_e in time_x_e_list:
            ...
            edge_index_t = torch.stack(
                [batch_query_dst[e_t_mask], batch_query_src[e_t_mask]]
            ).to(device)
            ...
            t_x_e_tensor = (
                torch.LongTensor([t_x_e])[None, :].expand(num_queries, -1).to(device)
            )
        ...
        for i in range(len(edge_index_list)):
            ...
            edge_index_l_i = torch.stack(
                [query_dst_list[i][e_t_mask_i], query_src_list[i][e_t_mask_i]]
            )
    ...
    def get_batch_A(
        self, num_nodes_cumsum, edge_index_list, device, return_edge_index=False
    ):
    ...
    def get_batch_A_n2g(self, num_nodes_cumsum, device):
        ...
        for i in range(batch_size):
            nids.append(
                torch.arange(num_nodes_cumsum[i], num_nodes_cumsum[i + 1]).long()
            )
            gids.append(
                torch.ones(num_nodes_cumsum[i + 1] - num_nodes_cumsum[i])
                .fill_(i)
                .long()
            )
    ...
    def sample(
        ...
        edge_index_list = [
            torch.LongTensor([[], []]).to(device) for _ in range(batch_size)
        ]
        ...
        level = 0.0
        abs_level_list = [torch.tensor([[level]]).to(device) for _ in range(batch_size)]
        ...
        num_nodes_cumsum = torch.cumsum(
            torch.tensor([0] + [len(x_n_i) for x_n_i in x_n_list]), dim=0
        )
        while True:
