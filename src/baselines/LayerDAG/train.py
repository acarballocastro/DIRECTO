...
def main(config: DictConfig):
    ...
    assert (
        dataset not in ["ba_dag", "er_dag", "er", "sbm", "planar"]
        or not config["general"]["conditional"]
    ), f"Conditional training is not supported for {dataset} dataset."
    config_df = pd.json_normalize(config, sep="/")

    ts = time.strftime("%b%d-%H:%M:%S", time.gmtime())

    wandb.init(
        project=f"dirgen_LayerDAG",
    ...
    train_node_count_dataset = LayerDAGNodeCountDataset(
        train_set, config["general"]["conditional"]
    )
    val_node_count_dataset = LayerDAGNodeCountDataset(
        val_set, config["general"]["conditional"]
    )

    train_node_pred_dataset = LayerDAGNodePredDataset(
        train_set, config["general"]["conditional"]
    )
    ...
    train_edge_pred_dataset = LayerDAGEdgePredDataset(
        train_set, config["general"]["conditional"]
    )
    val_edge_pred_dataset = LayerDAGEdgePredDataset(
        val_set, config["general"]["conditional"]
    )
    ...
    model_config = {
        ...
        "max_level": max(
            train_node_pred_dataset.input_level.max().item(),
    ...
    save_path = os.path.join(root_path, f"outputs/model_{dataset}_{ts}.pth")
...
def eval_node_count(device, val_loader, model):
    ...
    for batch_data in tqdm(val_loader):
        ...
        batch_A = dglsp.spmatrix(batch_edge_index, shape=(num_nodes, num_nodes)).to(
            device
        )
        batch_x_n = batch_x_n.to(device)
        batch_abs_level = batch_abs_level.to(device)
        batch_rel_level = batch_rel_level.to(device)
        batch_A_n2g = dglsp.spmatrix(batch_n2g_index, shape=(batch_size, num_nodes)).to(
            device
        )
        batch_label = batch_label.to(device)

        batch_logits = model(
            batch_A, batch_x_n, batch_abs_level, batch_rel_level, batch_A_n2g, batch_y
        )
...
def main_node_count(device, train_set, val_set, model, config, patience):
    ...
    val_loader = DataLoader(
        val_set, shuffle=False, collate_fn=collate_node_count, **config["loader"]
    )
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_data in tqdm(train_loader):
            ...
            # Check that it is possible to pass to dense
            try:
                batch_A.to_dense()
            ...
            batch_A_n2g = dglsp.spmatrix(
                batch_n2g_index, shape=(batch_size, num_nodes)
            ).to(device)
            batch_label = batch_label.to(device)
...
def eval_node_pred(device, val_loader, model):
    ...
    for batch_data in tqdm(val_loader):
        ...
        for d in range(D):
            ...
            batch_nll_d = batch_nll_d[
                torch.arange(batch_num_queries).to(device), batch_z[:, d]
            ]
...
def main_node_pred(device, train_set, val_set, model, config, patience):
    train_loader = DataLoader(
        train_set, shuffle=True, collate_fn=collate_node_pred, **config["loader"]
    )
    val_loader = DataLoader(val_set, collate_fn=collate_node_pred, **config["loader"])
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), **config["optimizer"])
...
def eval_edge_pred(device, val_loader, model):
    ...
    for batch_data in tqdm(val_loader):
        ...
        batch_nll = batch_nll[torch.arange(batch_num_queries).to(device), batch_label]
        total_nll += batch_nll.sum().item()
...
def main_edge_pred(device, train_set, val_set, model, config, patience):
    train_loader = DataLoader(
        train_set, shuffle=True, collate_fn=collate_edge_pred, **config["loader"]
    )
    val_loader = DataLoader(val_set, collate_fn=collate_edge_pred, **config["loader"])
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), **config["optimizer"])
