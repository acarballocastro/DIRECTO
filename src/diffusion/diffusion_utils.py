...
def sample_discrete_features(probX, probE, node_mask, directed=False):
    ...
    E_t[:, torch.arange(n), torch.arange(n)] = 0  # avoiding self-loops
...
def sample_discrete_feature_noise(limit_dist, node_mask, directed=False):
    ...
    # Getting all edge noise without
    diagonal_mask = torch.eye(n_max, dtype=torch.bool).unsqueeze(0).expand(bs, -1, -1)
    U_E[diagonal_mask] = 0
    ...
    return PlaceHolder(X=U_X, E=U_E, y=U_y).mask(node_mask, directed=directed)
