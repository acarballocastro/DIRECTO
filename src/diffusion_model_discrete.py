...
from src.models.transformer_model_directed import GraphTransformerDirected
...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def training_step(self, data, i):
        ...
        # end_extra = time.time()
        # extra_time = end_extra - start_extra
        # self.print(f"Time for computing extra data: {extra_time:.6f} seconds")

        # start_forward = time.time()
        pred = self.forward(
            noisy_data, extra_data, node_mask, data.edge_index, data.batch
        )
        # end_forward = time.time()
        # forward_time = end_forward - start_forward
        # self.print(f"Time for forward pass: {forward_time:.6f} seconds")
    ...
    def on_test_epoch_end(self) -> None:
        ...
        # Separate in num_sample_fold
        samples = [samples[i : i + self.cfg.general.final_model_samples_to_generate] for i in range(0, len(samples), self.cfg.general.final_model_samples_to_generate)]

        # Compute sampling metrics for each fold
        to_log = {}
        for i, fold in enumerate(samples):
            ...
            if i == 0:
                to_log = {i: [metrics[i]] for i in metrics}
            else:
                to_log = {i: to_log[i] + [metrics[i]] for i in metrics}

        # Compute mean and std across folds
        final_to_log = {i: [np.mean(to_log[i]), np.std(to_log[i])] for i in to_log}
        to_log.update(final_to_log)

        print(to_log)

        # Save results
        with open("sampling_metrics.txt", "w") as f:
    ...
    def reconstruction_logp(self, t, X, E, node_mask):
        ...
        sampled0 = diffusion_utils.sample_discrete_features(
            probX=probX0, probE=probE0, node_mask=node_mask, directed=self.directed
        )
        ...
        sampled_0 = utils.PlaceHolder(X=X0, E=E0, y=y0).mask(
            node_mask, directed=self.directed
        )
        ...
        edge_index, batch = to_sparse(sampled_0.X, sampled_0.E, sampled_0.y, node_mask)
        pred0 = self.forward(noisy_data, extra_data, node_mask, edge_index, batch)
    ...
    def apply_noise(self, X, E, y, node_mask):
        ...
        sampled_t = diffusion_utils.sample_discrete_features(
            probX=probX, probE=probE, node_mask=node_mask, directed=self.directed
        )
    ...
    def forward(self, noisy_data, extra_data, node_mask, edge_index=None, batch=None):
        X = torch.cat((noisy_data["X_t"], extra_data.X), dim=2).float()
        E = torch.cat((noisy_data["E_t"], extra_data.E), dim=3).float()
        y = torch.hstack((noisy_data["y_t"], extra_data.y)).float()
        return self.model(X, E, y, node_mask, edge_index, batch)
    ...
    def sample_batch(
        ...
        z_T = diffusion_utils.sample_discrete_feature_noise(
            limit_dist=self.limit_dist, node_mask=node_mask, directed=self.directed
        )
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt(
                s_norm, t_norm, X, E, y, node_mask, self.directed
            )
        ...
        if self.visualization_tools is not None:
            ...
            for i in range(num_molecules):
                ...
                self.print(
                    "\r{}/{} complete".format(i + 1, num_molecules), end=""
                )  # , flush = True
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, directed):
        ...
        # Convert to sparse representation
        edge_index, batch = to_sparse(X_t, E_t, y_t, node_mask)
        pred = self.forward(noisy_data, extra_data, node_mask, edge_index, batch)
        ...
        sampled_s = diffusion_utils.sample_discrete_features(
            prob_X, prob_E, node_mask=node_mask, directed=self.directed
        )
        ...
        return out_one_hot.mask(node_mask, directed=self.directed).type_as(
            y_t
        ), out_discrete.mask(node_mask, collapse=True, directed=self.directed).type_as(
            y_t
        )
