...
def main(cfg: DictConfig):
    ...
    if dataset_config["name"] in ["sbm", "comm20", "planar"]:
    ...
    elif dataset_config["name"] in ["tpu_tile"]:
    ...
    elif dataset_config["name"] in ["synthetic"]:
        from src.datasets.synthetic_dataset import (
            SyntheticDataModule,
            SyntheticDatasetInfos,
        )
        from src.analysis.directed_utils import SyntheticSamplingMetrics
        from analysis.visualization import DirectedVisualization, DAGVisualization
        ...
        sampling_metrics = SyntheticSamplingMetrics(
            ...
            metrics_list=metrics_dictionary[dataset_config["graph_type"]],
            graph_type=dataset_config["graph_type"],
        )
        dataset_infos = SyntheticDatasetInfos(datamodule, dataset_config)
        train_metrics = (
            TrainAbstractMetricsDiscrete()
        )

        visualization_tools = (
            DAGVisualization(cfg) if cfg.dataset.acyclic else DirectedVisualization(cfg)
        )
